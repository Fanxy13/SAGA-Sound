...
- **Eigene Songs:** Bibliothek → *Dateien* → Hochladen, oder MP3/M4A/WAV/OGG/FLAC einfach aufs Fenster ziehen. Titel, Künstler und Cover kommen aus den ID3-Tags (sonst aus dem Dateinamen), die Wellenform wird beim Import berechnet. Die Dateien bleiben in diesem Browser (IndexedDB) und sind nicht im Backup.
- **Hintergrund:** Browser starten in einem versteckten Tab keinen neu geladenen SoundCloud-Player. SagaSound spielt SoundCloud-Songs deshalb innerhalb einer Sammlung (Likes, Playlist, Uploads, Quellen der Empfehlungen) und springt dort ohne Neuladen weiter. Liegt der nächste Song der Warteschlange woanders, geht es im Hintergrund mit passenden Songs aus derselben Sammlung weiter, bis der Tab wieder sichtbar ist. Eigene Dateien laufen über einen normalen Audio-Player und spielen im Hintergrund ohne Einschränkung.
